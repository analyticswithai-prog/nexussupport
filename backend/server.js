...
function h(fn){return(req,res)=>fn(req,res).catch(err=>{console.error(err);res.status(500).json({error:'Internal server error'});});}
app.post('/api/auth/login',h(async(req,res)=>{
  ...
  const tenant=user.tenantId?await db.getTenant(user.tenantId):null;
...
app.get('/api/auth/me',auth,h(async(req,res)=>{const user=await db.getUserById(req.user.userId);const tenant=user?.tenantId?await db.getTenant(user.tenantId):null;res.json({user:{id:user.id,name:user.name,email:user.email,role:user.role,tenantId:user.tenantId},tenant});}));
app.get('/api/tenants',auth,h(async(req,res)=>{if(req.user.role!=='superadmin')return res.status(403).json({error:'Superadmin only'});res.json(await db.getAllTenants());}));
app.get('/api/tenants/:tenantId',auth,tenantGuard,h(async(req,res)=>{const t=await db.getTenant(req.params.tenantId);if(!t)return res.status(404).json({error:'Not found'});res.json(t);}));
app.put('/api/tenants/:tenantId/settings',auth,tenantGuard,h(async(req,res)=>{if(!['admin','superadmin'].includes(req.user.role))return res.status(403).json({error:'Admins only'});const t=await db.getTenant(req.params.tenantId);if(!t)return res.status(404).json({error:'Not found'});const updated=await db.updateTenantSettings(t.id,{...t.settings,...req.body});res.json(updated);}));
app.get('/api/tenants/:tenantId/dashboard',auth,tenantGuard,h(async(req,res)=>{
  const tid=req.params.tenantId;const convos=await db.getConversationsByTenant(tid);
...
app.get('/api/tenants/:tenantId/conversations',auth,tenantGuard,h(async(req,res)=>{
  const{status,channel,search,page=1,limit=20}=req.query;
  let list=await db.getConversationsByTenant(req.params.tenantId);
...
app.get('/api/tenants/:tenantId/conversations/:convId',auth,tenantGuard,h(async(req,res)=>{const conv=await db.getConversation(req.params.convId);if(!conv||conv.tenantId!==req.params.tenantId)return res.status(404).json({error:'Not found'});res.json({...conv,messages:buildMessages(conv)});}));
app.get('/api/tenants/:tenantId/agents',auth,tenantGuard,h(async(req,res)=>{res.json(await db.getAgentsByTenant(req.params.tenantId));}));
app.get('/api/tenants/:tenantId/analytics',auth,tenantGuard,h(async(req,res)=>{
  const convos=await db.getConversationsByTenant(req.params.tenantId);
